...
#define Z_HOME_OFFSET 11570 // in counts, from home to collet lined up with lower rollers
...
#define SASH_OFFSET -231 // counts, laser broken to tip of sash
...
void loop() {
	...
	XMoveDistance(1380*8); // should eject stick
...
void revHomeStick() { // flawed, requires laser beam to be broken by current stick
	while (digitalRead(beam2Pin)) {
		Xaxis.MoveVelocity(1000*8);
	}
	Xaxis.MoveStopAbrupt();
...
void setupPins() {
  ...
  // }
	...
	if (laserError) {Serial.println("Error during laser test");}
	else {Serial.println("Laser test passed");}
	digitalWrite(laser1Pin, HIGH);
	digitalWrite(laser2Pin, HIGH);
	digitalWrite(laser3Pin, HIGH);
}